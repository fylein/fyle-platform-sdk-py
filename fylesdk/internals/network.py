...
class Network:
	...
	def _assert_response(self, response):
		if response.status_code == 400:
			assertions.assert_valid(False, 'Some of the parameters are wrong, Response: {}'.format(response.text))
		elif response.status_code == 401:
			assertions.assert_auth(None, 'Invalid token, try to refresh it, Response: {}'.format(response.text))
		elif response.status_code == 403:
			assertions.assert_true(False, 'Forbidden, the user has insufficient privilege, Response: {}'.format(response.text))
		elif response.status_code == 404:
			assertions.assert_found(False, 'Not found item with ID, Response: {}'.format(response.text))
		elif response.status_code == 498:
			assertions.assert_auth(False, 'Expired token, try to refresh it, Response: {}'.format(response.text))
