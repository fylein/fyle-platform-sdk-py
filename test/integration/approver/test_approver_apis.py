...
def test_list_all_categories(fyle, mock_data):
  ...
  categories_generator = fyle.v1.approver.categories.list(query_params=query_params)
  mock_categories = mock_data.categories.get()

  if categories_generator['data']:
    assert dict_compare_keys(categories_generator['data'][0], mock_categories[0]) == [], 'fyle.v1.approver.categories.list_all() has stuff that mock_data doesnt'
...
def test_list_reports(fyle, mock_data):
  ...
  reports_generator = fyle.v1.approver.reports.list(query_params=query_params)
  ...
  if reports_generator["data"]:
    ...
    for reports in reports_generator["data"]:
      if reports["state"] == "APPROVER_PENDING":
        ...
        break
    assert dict_compare_keys(reports_generator["data"][0], mock_reports[0]) == [], 'fyle.v1.approver.reports.list() has stuff that mock_data doesnt'
...
def test_approve_report(fyle, mock_data):
  approve_report = {}
  try:
    approve_report = fyle.v1.approver.reports.approve(id_=report_id)
    mock_reports = mock_data.reports.get()
  except:
    logger.error("report is not in APPROVER_PENDING state")
  if approve_report:
    assert dict_compare_keys(approve_report["data"], mock_reports[0]) == [], 'fyle.v1.approver.reports.list() has stuff that mock_data doesnt'
...
def test_report_get_by_id(fyle, mock_data):
  reports_generator = fyle.v1.approver.reports.get_by_id(id_=report_id)
  mock_reports = mock_data.reports.get()
  if reports_generator:
    assert dict_compare_keys(reports_generator["data"], mock_reports[0]) == [], 'fyle.v1.approver.reports.list() has stuff that mock_data doesnt'
