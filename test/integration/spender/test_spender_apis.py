...
def test_get_profile(fyle, mock_data):
  profile = fyle.v1.spender.my_profile.get()
  mock_profile = mock_data.profile.get()

  if profile:
    assert dict_compare_keys(profile["data"], mock_profile) == [], 'fyle.v1.spender.profile.list_all() has stuff that mock_data doesnt'
...
def test_get_by_id_profile(fyle, mock_data):
  try:
    profile = fyle.v1.spender.my_profile.get_by_id(id_="123456")
  except:
    logger.error("Api Not implemented")  


def test_create_file(fyle, mock_data):
  create_file = fyle.v1.spender.files.create_file(payload = {
    "data": {
      "name": "sample.jpg",
      "type": "RECEIPT",
  ...
  if create_file["data"]: 
    ...
    assert dict_compare_keys(create_file["data"], mock_files) == [], 'response from fyle.v1.spender.files.create_file() has stuff that mock_data doesnt'
...
def test_generate_file_urls(fyle, mock_data):
  generate_file_urls = fyle.v1.spender.files.generate_file_urls(payload = {
    "data": {
        "id": file_id
      }
  })
  ...
  if generate_file_urls["data"]:
    ...
    assert dict_compare_keys(generate_file_urls["data"], mock_files) == [], 'response from fyle.v1.spender.files.create_file() has stuff that mock_data doesnt'
...
def test_put_file_to_url(fyle):
  ...
  put_file_to_url = fyle.v1.spender.files.put_file_to_url(
...
def test_create_expense(fyle, mock_data):
  create_expense = fyle.v1.spender.expenses.create_expense(payload = {
    "data": {
  ...
  if create_expense["data"]:
    ...
    assert dict_compare_keys(create_expense["data"], mock_files) == [], 'response from fyle.v1.spender.expenses.create_expense() has stuff that mock_data doesnt'
...
def test_attach_receipt(fyle, mock_data):
  attach_receipt = fyle.v1.spender.expenses.attach_receipt(payload = {
    "data": {
      "id": expense_id,
      "file_id": file_id
    }
  ...
  if attach_receipt["data"]:
    assert dict_compare_keys(attach_receipt["data"], mock_files) == [], 'response from fyle.v1.spender.expenses.attach_receipt() has stuff that mock_data doesnt'
